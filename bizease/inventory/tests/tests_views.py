...
class InventoryViewsTest(APITransactionTestCase):
	def setUp(self):
		...
		self.item_4 = Inventory.objects.create(owner=self.test_user, product_name="Safety Boots", price=45000, stock_level=20, category="ppe")
		self.item_5 = Inventory.objects.create(owner=self.test_user, product_name="Helmet", price=8000, stock_level=40, category="ppe")
		self.item_6 = Inventory.objects.create(owner=self.test_user, product_name="Biscuits", price=100, stock_level=36)

	def create_product_that_exists(self):
		response = self.client.post(reverse("inventory", args=["v1"]), 
			{"product_name": self.item_1.product_name, "stock_level": 10, "price": 100000}, format='json')
	...
	def create_valid_new_product(self):
		response = self.client.post(reverse("inventory", args=["v1"]), {"product_name": "Rice", "stock_level": 50, "price": 100000}, format='json')
	...
	def test_create_inventory_items_without_credentials(self):
		response = self.client.post(reverse("inventory", args=["v1"]), {"product_name": "product-1", "stock_level": 5, "price": 800}, format='json')
