...
class OrdersViewsTest(APITransactionTestCase):
	...
	def get_orders_by_status(self):
		...
		self.assertEqual(response.data["data"]["length"], 2)
		self.assertEqual(response.data["data"]["orders"][0]["client_name"], "gumball") # gumball comes first because of the default ordering order
		self.assertEqual(response.data["data"]["orders"][1]["client_name"], "bob")
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def get_orders_ordered_by_order_date(self):
		...
		self.assertEqual(response.data["data"]["length"], 4)
		self.assertEqual(response.data["data"]["orders"][0]["client_name"], "bob")
		self.assertEqual(response.data["data"]["orders"][3]["client_name"], "Batman")
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_get_orders_with_credentials_and_query_params(self):
		...
		item_1 = Inventory.objects.create(owner=self.test_user, product_name="Bag", price=16000, stock_level=75)
		item_2 = Inventory.objects.create(owner=self.test_user, product_name="Tape", price=6000, stock_level=85)
		item_3 = Inventory.objects.create(owner=self.test_user, product_name="Head Phones", price=9000, stock_level=100)

		order_1 = Order(product_owner_id=self.test_user, client_name="prismo", client_email="prismo@gmail.com", status="Delivered")
		order_2 = Order(product_owner_id=self.test_user, client_name="gumball", client_email="gumballwatterson@elmoremail.com")
		order_3 = Order(product_owner_id=self.test_user, client_name="Batman", client_email="brucewayne@wayne.gotham", status="Delivered")
	...
	def test_get_order_stats(self):
		self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.access_token)
		new_order = Order(product_owner_id=self.test_user, client_name="bmo", client_email="bmo@nomail.com", status="Delivered")
...
class OrderedProductViewTest(APITransactionTestCase):
	def setUp(self):
		...
		self.item = Inventory.objects.create(owner=self.user, product_name="Cup", price=800, stock_level=100)

		self.order = Order(product_owner_id=self.user, client_name="bob", client_email="bob@gmail.com")
	...
	def test_add_new_ordered_product_to_order(self):
		Inventory.objects.create(owner=self.user, product_name="Plate", price=1500, stock_level=100)
